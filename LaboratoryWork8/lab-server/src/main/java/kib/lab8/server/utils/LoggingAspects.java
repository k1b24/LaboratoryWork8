    ...
    public class LoggingAspects {
        ...
        @AfterReturning(pointcut = "execution(kib.lab8.common.util.client_server_communication.requests.CommandRequest listen())", returning = "result")
        ...
        @AfterReturning(pointcut = "execution(Object executeCommandFromRequest(kib.lab8.common.util.client_server_communication.requests.CommandRequest))", returning = "result")
        ...
        @AfterReturning(pointcut = "execution(String sendResponse(kib.lab8.common.util.client_server_communication.responses.CommandResponse))", returning = "result")
        public void sendResponseAdvise(Object result) {
            LOGGER.info("Ответ отправлен клиенту " + result);
        }

        @AfterReturning(pointcut = "execution(public * kib.lab8.server.utils.CollectionManager.*(..))")
        ...
        @AfterReturning(pointcut = "execution(public * kib.lab8.server.db_utils.DBManager.*(..))")
